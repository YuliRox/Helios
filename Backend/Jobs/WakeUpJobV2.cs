...
namespace Helios.Jobs
{
    public class WakeUpJobV2 : DimmerJob
    {
        ...
        protected override async Task ExecuteInternal(IJobExecutionContext context)
        {
            ...
            var percentage = this.HeliosOptions.DimmerMinimumPercentage;
            var percentageDiff = 100.0 - (double)percentage;
            var timeFrame = (double)this.HeliosOptions.DimmerTime;
            ...
            {
                var percentageMessage = await this.ManagedDimmer.SetPercentage(percentage);
                percentage++;
                await Task.Delay(timeIntervall);

                if (session.IsInterrupted(Logger, this.HeliosOptions.DimmerPercentageStatusTopic, percentageMessage))
                {
